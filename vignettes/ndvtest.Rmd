---
title: "Implementation of Shi's non-degeranate Vuong test for non-nested models"
...
vignette: >
  %\VignetteIndexEntry{Implementation of Shi's non-degeranate Vuong test for non-nested models}
...
competing models, $F_\firstp = \left\{f(y|z; \firstp); \firstp \in
\beta\right\}$ and $G_\secondp = \left\{g(y|z; \secondp); \secondp \in
\Gamma\right\}$. Denoting $h(y | z)$ the true conditional density.
The distance of $F_\firstp$ from the true model is measured by the
minimum KLIC:
...
distribution of $(y, X)$ and $\firstp_*$ is the pseudo-true value of
$\firstp$^[$\firstp_*$ is called the pseudo-true value because $f$ may
be an uncorrect model.]. As the true model is unobserved, denoting
$\wholep^\top = (\firstp ^ \top, \secondp ^ \top)$, we consider the
difference of the KLIC distance to the true model of model $G_\secondp$
and model $F_\firstp$:


$$ \Lambda(\wholep) = D_g - D_f = \mbox{E}^0\left[\ln f(y\mid z;
\firstp_*)\right]- \mbox{E}^0\left[\ln g(y\mid z; \secondp_*)\right] =
\mbox{E}^0\left[\ln \frac{f(y\mid z; \firstp_*)}{g(y\mid z;
\secondp_*)}\right] $$
...
hypothesis is either $\Lambda>0$, which means that $F_\firstp$ is
better than $G_\secondp$ or $\Lambda<0$, which means that $G_\secondp$
is better than $F_\firstp$. Denoting, for a given random sample of
size $N$, $\hat{\firstp}$ and $\hat{\secondp}$ the maximum likelihood
estimators of the two models and $\ln L_f(\hat{\firstp})$ and $\ln
L_g(\hat{\secondp})$ the maximum value of the log-likelihood functions
of respectively $F_\firstp$ and $G\secondp$, $\Lambda$ can be
consistently estimated by:

$$
\hat{\Lambda}_N = \frac{1}{N} \sum_{n = 1} ^ N  \left(\ln f(y_n \mid
x_n, \hat{\firstp}) - \ln g(y_n \mid x_n, \hat{\secondp})\right)
= \frac{1}{N} \left(\ln L_f(\hat{\firstp}) - \ln L_g(\hat{\secondp})\right)
$$
...
nested models is $2 \left(\ln L^f(\hat{\firstp}) - \ln
L^g(\hat{\secondp})\right)$, which is $2 N \hat{\Lambda}_N$.

The variance of $\Lambda$ is:

$$
\omega^2_* = \mbox{V}^o \left[\ln \frac{f(y \mid x; \firstp_*)}{g(y
\mid x; \secondp_*)}\right]
$$
...
x_n, \hat{\firstp}) - \ln g(y_n \mid x,_n \hat{\secondp})\right) ^ 2 - 
\hat{\Lambda}_N ^ 2
$$
...
$$
A(\wholep_*) = \mbox{E}^0\left[\frac{\partial^2 \Lambda}{\partial \wholep
\partial \wholep ^ \top}\right] = 
\mbox{E}^0\left[\begin{array}{cc}
\frac{\partial^2 \ln f}{\partial \firstp \partial \firstp ^
\top} & 0 \\
...
\wholep}\frac{\partial \Lambda}{\partial \wholep ^ \top}\right]=
\mbox{E}^0\left[
\left(\frac{\partial \ln f}{\partial \firstp}, 
- \frac{\partial \ln g}{\partial \secondp} \right)
\left(\frac{\partial \ln f}{\partial \firstp ^ \top}, 
- \frac{\partial \ln g}{\partial \secondp ^ \top} \right)
\right]
= \mbox{E}^0\left[
\begin{array}{cc}
\frac{\partial \ln f}{\partial \firstp} \frac{\partial \ln f}{\partial
\firstp^\top} & 
- \frac{\partial \ln f}{\partial \firstp} \frac{\partial \ln g}{\partial
\secondp ^ \top} \\
- \frac{\partial \ln g}{\partial \secondp} \frac{\partial \ln f}{\partial
  \firstp^\top} & 
\frac{\partial \ln g}{\partial \secondp} \frac{\partial \ln g}{\partial \secondp^\top}
\end{array}
\right]
$$
...
\left[
\begin{array}{cc}
B_f(\firstp_*) & - B_{fg}(\firstp_*, \secondp_*) \\
- B_{gf}(\firstp_*, \secondp_*) & B_g(\secondp_*)
\end{array}
\right]
$$
...
$$
W(\wholep_*) =  B(\wholep_*) \left[-A(\wholep_*)\right] ^ {-1}= 
\left[
\begin{array}{cc}
-B_f(\firstp_*) A^{-1}_f(\firstp_*) & - B_{fg}(\firstp_*, \secondp_*)
A^{-1}_g(\secondp_*) \\
B_{gf}(\secondp_*, \firstp_*) A^{-1}_f(\firstp_*) & B_g(\secondp_*)
A^{-1}_g(\secondp_*)
\end{array}
\right]
$$
...
models for $\hat{\wholep}$. For example, 

$$
\hat{A}_f(\hat{\firstp}) = \frac{1}{N}
\sum_{n= 1} ^ N \frac{\partial^2 \ln f}{\partial \firstp \partial
\firstp ^ \top}(\hat{\firstp}, x_n, y_n)
$$

$$ \hat{B}_{fg}(\hat{\wholep})= \frac{1}{N} \sum_{n=1}^N
\frac{\partial \ln f}{\partial \firstp}(\hat{\firstp}, x_n, y_n)
\frac{\partial \ln g}{\partial \secondp^\top}(\hat{\secondp}, x_n, y_n)
$$
...
\left(
\begin{array}{cc} 
\sigma z_\omega - z_\wholep ^ \top V z_\wholep / 2 \\ 
\sigma ^ 2 - 2 \sigma \rho_* ^ \top V z_\wholep + z_\wholep ^ \top V ^ 2
z_\wholep
\end{array}
\right)
$$
...
$$
\left(\begin{array}{c}z_\omega \\ z_\wholep \end{array}\right) \sim
...
The `nonnest2` package provides a `vuongtest` function that implements
...
The `ndvtest` package provides the `ndvtest` function. As for the
...
`N` is the sample size.  Therefore, the `ndvtest` function can be used
...
method. They are provided in the `ndvtest` package as `turnout`, a
...
using `ndvtest` by setting the argument `vartest` to `TRUE`:
